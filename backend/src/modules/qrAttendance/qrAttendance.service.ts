...
export class QRAttendanceService {
   ...
   */
  static async generateQRCode(userId: string): Promise<{ qrCodeDataUrl: string; qrData: QRCodeData }> {
   ...
   */
  private static async createAttendanceRecord(employeeId: string, validatedByUserId: string): Promise<string> {
    try {
   ...
   */
  static async validateQRCode(qrData: QRCodeData, scannedByUserId: string): Promise<{ isValid: boolean; userId?: string; attendanceId?: string; error?: string }> {
    try {
      ...
      const storedQRData: QRCodeData = JSON.parse(storedData);
