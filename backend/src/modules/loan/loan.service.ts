...
export class LoanService {
   ...
   */
  static async createLoan(companyId: string, data: CreateLoanInput, createdBy: string) {
   ...
   */
  static async getLoansByCompany(companyId: string, filters?: { employeeId?: string; status?: string }) {
    const where: any = {
      employee: {
        companyId
      }
   ...
   * Get loan by ID
   */
  static async getLoanById(companyId: string, loanId: string) {
   ...
   * Update a loan
   */
  static async updateLoan(companyId: string, loanId: string, data: UpdateLoanInput) {
    // First verify access
    await this.getLoanById(companyId, loanId);

    const updateData: any = {};
    if (data.amount !== undefined) updateData.amount = data.amount;
    if (data.monthlyDeduction !== undefined) updateData.monthlyDeduction = data.monthlyDeduction;
   ...
   * Delete a loan
   */
  static async deleteLoan(companyId: string, loanId: string) {
    // First verify access
    await this.getLoanById(companyId, loanId);
   ...
   */
  static async calculateLoanDeductions(employeeId: string): Promise<number> {
   ...
   */
  static async applyLoanDeductions(employeeId: string, amountPaid: number) {
    const activeLoans = await prisma.loan.findMany({
      where: {
        employeeId,
        status: 'ACTIVE',
    ...
    for (const loan of activeLoans) {
      ...
      const deductionForThisLoan = Math.min(
        Number(loan.monthlyDeduction),
        Number(loan.remainingAmount),
        remainingAmountToDeduct
      );
   ...
   */
  static async getActiveLoansForEmployee(employeeId: string) {
    return prisma.loan.findMany({
      where: {
        employeeId,
        status: 'ACTIVE',
