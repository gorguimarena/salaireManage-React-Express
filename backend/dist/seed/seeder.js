    ...
    try {
        ...
        for (const u of users) {
            ...
            if (!companyId && u.roleName !== "SUPER_ADMIN") {
                if (u.email.includes("companyone"))
                    companyId = companyIds["CompanyOne"];
                if (u.email.includes("companytwo"))
                    companyId = companyIds["CompanyTwo"];
            }
        ...
        for (const companyName of Object.keys(companyIds)) {
            ...
            for (const emp of employees) {
                if (!emp.salaryOrRate || !emp.contractType)
                    continue;
