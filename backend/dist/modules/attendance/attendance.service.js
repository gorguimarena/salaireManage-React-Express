...
export class AttendanceService {
    ...
    static async listByEmployee(companyId, employeeId, startDate, endDate) {
        const where = {
            workSchedule: {
                employee: {
                    id: employeeId,
    ...
    static async listByCompany(companyId, startDate, endDate) {
        const where = {
            workSchedule: {
                employee: {
                    companyId
    ...
    static async update(companyId, attendanceId, data) {
        // Vérifier l'accès
    ...
    static async validate(companyId, attendanceId) {
        return this.update(companyId, attendanceId, { validated: true });
    }
    static async bulkCreate(companyId, attendances) {
    ...
    static async markDailyAttendance(companyId, data) {
        ...
        const ineligibleEmployeeIds = data.presentEmployeeIds.filter(employeeId => !eligibleEmployees.some(emp => emp.id === employeeId));
        ...
        const newEmployeeIds = data.presentEmployeeIds.filter(employeeId => !alreadyMarkedEmployeeIds.includes(employeeId));
        ...
        const attendancesToCreate = newEmployeeIds
            ...
            if (!employee || employee.workSchedules.length === 0)
                return null;
    ...
    static async getTodayAttendances(companyId) {
     ...
     */
    static async validateAttendanceWithHours(companyId, attendanceId, validatedByUserId, workedHours) {
        // Find the attendance
        const attendance = await prisma.attendance.findUnique({
            where: { id: attendanceId },
            include: {
     ...
     */
    static async getPendingAttendances(companyId) {
        return prisma.attendance.findMany({
            where: {
                validated: false,
                workSchedule: {
